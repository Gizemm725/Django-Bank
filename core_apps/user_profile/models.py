...
class Profile(TimeStampedModel):
    ...
    # settings.AUTH_USER_MODEL kullan
    ...
    marital_status = models.CharField(_("Marital Status"), max_length=8, choices=MaritalStatus.choices, default=MaritalStatus.UNKNOWN)
    ...
    phone_number = PhoneNumberField(_("Phone Number"), max_length=30, default="+905521824416")
    ...
    employment_status = models.CharField(_("Employment Status"), max_length=20, choices=EmploymentStatus.choices, default=EmploymentStatus.UN_EMPLOYED)
    employer_name = models.CharField(_("Employer Name"), max_length=50, blank=True, null=True)
    annual_income = models.DecimalField(_("Annual Income"), max_digits=15, decimal_places=2, default=0.00)
    ...
    photo = CloudinaryField(_("Profile Photo"), blank=True, null=True)
    ...
    def clean(self) -> None:
        ...
        if self.id_issue_date and self.id_expiry_date:
            if self.id_issue_date >= self.id_expiry_date:
                raise ValidationError(_("ID issue date must be before expiry date."))
...
class NextOfKin(TimeStampedModel):
    ...
    def __str__(self) -> str:
        return f"{self.title} {self.first_name} {self.last_name} - Next of Kin for {self.profile.user.full_name}"
...
# User tanımını EN SONA taşıdım - circular import'u önlemek için
User = get_user_model()